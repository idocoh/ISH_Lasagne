...
def run_all(use_nn_classifier=False, folder_name=None, input_size_pre=None):
    ...
    folder_name = "CAE_" + str(amount_train) + "_50x30-" + str(time.time()) if folder_name is None else folder_name
    ...
    image_width = [160, 160, 200, 240, 300, 280, 320, 240, 320, 400, 160, 480, 120, 960, 100, 200, 200, 240, 80, 640, 800]
    image_height = [80, 100, 120, 120, 140, 140, 160, 120, 200, 240, 80,  240, 60,  480, 80,  160, 200, 160, 40, 320, 480]
    number_pooling_layers = [2, 2, 2, 2, 2, 2, 3, 3, 3, 3, 3, 3, 1, 4, 2, 2, 2, 2, 1, 4, 4]
    layers_size = [
        [8, 8, 8, 8, 8, 8, 8, 8, 8],
        [4, 4, 4, 4, 4, 4, 4, 4, 4],
    ...
    for zero_meaning in [False]:
        try:
            for input_size_index in [20, 18, 19, 13]:
                try:
                            ...
                            for to_shuffle_input in [False]:
                                try:
                                    for filter_type in [2, 1, 0]:  # range(2, -1, -2):
                                        try:
                                            for number_conv_layers in [2, 3, 4]:
                                                try:
                                                    for lr in [4, 5, 3, 2]: #, 1, 0, 6]:  # range(5, 1, -1):
                                                        try:
