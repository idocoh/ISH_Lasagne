...
def run_all():
    ...
    for i in range(1, 5, 1):
        print("Run #", i)
        # try:
        run(layers_size=[32, 32, 32, 64, 64, 64, 32, 32, 32], epochs=epochs, learning_rate=0.04+0.005*i, update_momentum=0.9,
                dropout_percent=input_noise_rate, loadedData=data, folder_name=folder_name, amount_train=amount_train,
                zero_meaning=zero_meaning, activation=None, last_layer_activation=tanh, filters_type=11, train_valid_split=0.005,
    ...
    for i in range(1, 5, 1):
        print("Run #", i)
        # try:
        run(layers_size=[32, 32, 64, 64, 64, 64, 64, 32, 32], epochs=epochs, learning_rate=0.04 + 0.005 * i,
            update_momentum=0.9,
            dropout_percent=input_noise_rate, loadedData=data, folder_name=folder_name, amount_train=amount_train,
            zero_meaning=zero_meaning, activation=None, last_layer_activation=tanh, filters_type=11,
            train_valid_split=0.005,
            svm_negative_amount=svm_negative_amount, batch_size=32)
