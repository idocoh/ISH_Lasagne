...
class LogisticRegression(object):
    ...
    def __init__(self, input, n_in, n_out):
        ...
        # plain-k
        self.p_y_given_x = T.dot(input, self.W) + self.b #IDO: T.nnet.softmax(T.dot(input, self.W) + self.b)
    ...
    def root_mean_square_error(self, y):
#         print 'in rmse y is-' 
#         print y.eval()
#         print 'p_y_given_x is '
#         print self.p_y_given_x
#         print (self.p_y_given_x - y).shape[0]
#         print T.norm((self.p_y_given_x - y),ord=2)
#         return T.mean(numpy.linalg.norm((self.p_y_given_x - y),ord=2,axis=1))
#         return T.sqrt(((self.p_y_given_x - y) ** 2).mean())
#         results, updates = theano.scan(lambda x_i: T.sqrt((x_i ** 2).sum()), (self.p_y_given_x - y))
#         print results
#         return theano.function(inputs=[], outputs=[T.mean(results)])
       
#         print (self.p_y_given_x-y).ndim
#         print ( T.sum( T.pow((self.p_y_given_x - y) ,2) ,axis=1)).ndim
#         print ( T.sqrt( T.sum( (self.p_y_given_x - y) * (self.p_y_given_x - y) ,axis=1) )).ndim
#         print (T.mean( T.sqrt( T.sum( (self.p_y_given_x - y) * (self.p_y_given_x - y) ) ) )).ndim
#         return T.mean( T.sqrt( T.sum( T.pow( (self.p_y_given_x - y) ,2) ,axis=1) ) )
#         return T.mean( T.sqrt( T.sum( (self.p_y_given_x - y) * (self.p_y_given_x - y) ,axis=1) ) )#/ ( 2 *  y.shape[0] ) ) )
        return T.sqrt( T.mean( T.sum( T.pow((self.p_y_given_x - y) ,2) ,axis=1) ) )
#         return T.mean(T.sqrt( ( T.sum( T.pow((self.p_y_given_x - y) ,2) ,axis=1) ) / y.shape[0] ))
...
#         print "in error y is "
#         print y.eval()
#         return T.mean(self.root_mean_square_error(y))
#         print numpy.linalg.norm((self.y_pred - y),ord=2,axis=1)
#         return T.mean(numpy.linalg.norm((self.y_pred - y),ord=2,axis=1))
