...
#     
        ...
        #counts how many wrong prediction (even one wrong class means the example predicted was wrong)
        valid_set_eror=((squared_error(network_output, expected_output) > 0.25).sum(axis=1)>0).sum()
        #counts how many wrong binary GO class
        valid_calsses_eror=(squared_error(network_output, expected_output) > 0.25).sum()
...
#         accuracy = T.mean(T.eq(prediction, expected_output), dtype=theano.config.floatX)  # @UndefinedVariable
        ...
        validation_function=theano.function(inputs=[x, expected_output],
                                      outputs=[valid_set_eror, valid_calsses_eror],#, accuracy, prediction],
                                      allow_input_downcast=True)
...
#             print train_loss
        ...
        total_accuracy_set = 0
        total_accuracy_class = 0
        num_of_batches = len(valid_set_x)/BATCH_SIZE
        for j in range(num_of_batches):
            batch_slice = slice(j*BATCH_SIZE, (j+1)*BATCH_SIZE)
            valid_set_eror,valid_calsses_eror = valid_function(valid_set_x[batch_slice], valid_set_y[batch_slice])
#             out_p = output_function(valid_set_x[batch_slice])
#             print out_p[:BATCH_SIZE]
#             print valid_set_y
            print valid_set_eror
            print valid_calsses_eror
#             print ac
##             print accuracy
##             if (i==NUM_OF_EPOCH-1):
##                 for k in range(len(out_p)):
##                     validation_file.write(str('%.5f' % out_p[k][1]) + '\n')
            total_accuracy_set += valid_set_eror
            total_accuracy_class +=valid_calsses_eror
    
    
        # evaluating error
        total_accuracy_set/=float(len(valid_set_x))
        print '~~~~~~~~~~~~~Validation Error is: {0}'.format(1-total_accuracy_set)
        validation_file.write('{0}\n'.format(1-total_accuracy_set))
        total_accuracy_class/=float(len(valid_set_x))
        print '~~~~~~~~~~~~~Validation Class Error is: {0}'.format(1-total_accuracy_class)
...
#     print out_p
    ...
    outputFile.write("error is: "+str(1-total_accuracy_set) + "\n")
...
if __name__ == "__main__":
    run(LEARNING_RATE=0.1, NUM_OF_EPOCH=35, NUM_UNITS_HIDDEN_LAYER=[5, 10, 20, 40], BATCH_SIZE=5, toShuffleInput = False , withZeroMeaning = False)
    run(LEARNING_RATE=0.07, NUM_OF_EPOCH=35, NUM_UNITS_HIDDEN_LAYER=[5, 10, 20, 40], BATCH_SIZE=5, toShuffleInput = False , withZeroMeaning = False)    
    run(LEARNING_RATE=0.09, NUM_OF_EPOCH=35, NUM_UNITS_HIDDEN_LAYER=[5, 10, 20, 40], BATCH_SIZE=5, toShuffleInput = False , withZeroMeaning = True)
    run(LEARNING_RATE=0.2, NUM_OF_EPOCH=35, NUM_UNITS_HIDDEN_LAYER=[5, 10, 20, 40], BATCH_SIZE=5, toShuffleInput = False , withZeroMeaning = False)
    run(LEARNING_RATE=0.04, NUM_OF_EPOCH=35, NUM_UNITS_HIDDEN_LAYER=[5, 10, 20, 40], BATCH_SIZE=5, toShuffleInput = False , withZeroMeaning = True)
